...
void ofile(int moral,int health, int crossed, int obey){
  ofstream mfile;
  mfile.open("results.txt");
  mfile << " ---RESULTS--- \n ";
  mfile << "Morality: " << moral << "\n Health: " << health << "\n Crossed: " << crossed << "| Obeyed: " << obey;
  mfile << "\n\n";
  mfile.close();
}
...
void game(string OS){
  ...
  cout << "Save(1. Yes, 2. No)? ";
  cin >> save;

  if(save == 1){
    ofile(data[0], data[3], data[1], data[2]);
  }
  else{
